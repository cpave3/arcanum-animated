...
class TurnUndeadTests(unittest.TestCase):
    ...
    def test_public_render_endpoints_palette_alpha_and_metadata(self):
        self.assertEqual((self.effect.size, self.effect.fps, self.effect.frames), (640, 30, 120))
        ...
        np.testing.assert_array_equal(self.pixels(self.effect.poster_frame), self.pixels(self.effect.poster_frame))
    ...
    def test_cue_is_a_central_flash_and_poster_has_a_broad_blast(self):
        ...
        self.assertLess(self.pixels(self.effect.frames-5)[..., 3].sum(), poster.sum()*.01)

    def test_motion_expands_radially_without_a_projectile_or_residue(self):
        radii = []
        for frame in (self.effect.cue_frame+offset for offset in (6, 15, 24, 34, 46)):
    ...
    def test_runes_arrive_one_by_one_then_charge_and_launch_intact(self):
        ...
        def sample(frame):
            ...
            calls = [call.kwargs for call in spy.call_args_list]
            self.assertTrue(all(call['fragmentation'] == 0 for call in calls))
            return calls, image

        radius = lambda call: np.linalg.norm(np.asarray(call['center'])-256)
        for index in range(len(runes.GLYPHS)):
            early, early_image = sample(index*4+2)
            self.assertEqual(sum(call['opacity'] > 0 for call in early), index+1)
            self.assertGreater(early[index]['opacity'], 0)
            self.assertLess(early[index]['opacity'], 1)
            settled, settled_image = sample(index*4+8)
            self.assertAlmostEqual(radius(settled[index]), 55)
            self.assertGreater(radius(early[index]), 55)
            self.assertEqual(settled[index]['opacity'], 1)
            self.assertGreater(settled[index]['rotation'], early[index]['rotation'])
            self.assertFalse(np.array_equal(early_image, settled_image))

        assembled, _ = sample(36)
        charged, _ = sample(self.effect.cue_frame-1)
        self.assertTrue(all(call['opacity'] == 1 for call in assembled))
        for a, b in zip(assembled, charged):
            self.assertAlmostEqual(radius(a), 55)
            self.assertAlmostEqual(radius(b), 55)
            self.assertGreater(b['rotation']-a['rotation'], 2*np.pi)
        for before, after in ((0, 6), (6, 20), (20, 36)):
            earlier, _ = sample(self.effect.cue_frame+before)
            later, _ = sample(self.effect.cue_frame+after)
    ...
    def test_echoes_are_independently_timed_visible_layers(self):
        ...
        with patch.object(turn_undead, 'radiant_echo', wraps=original) as spy:
            complete = self.pixels(self.effect.poster_frame)
        ...
        for start in starts:
            ...
            with patch.object(turn_undead, 'radiant_echo', side_effect=omit):
                removed = self.pixels(self.effect.poster_frame)
    ...
    def test_shared_painters_and_field_have_observable_contributions(self):
        ...
        for module, name, replacement in cases:
            with self.subTest(primitive=name):
                ...
                with patch.object(module, name, autospec=True, side_effect=original) as spy:
                    rendered = self.pixels(self.effect.poster_frame if name != 'ring' else 8)
                    self.assertTrue(spy.called)
                with patch.object(module, name, return_value=replacement):
                    removed = self.pixels(self.effect.poster_frame if name != 'ring' else 8)
        ...
        with patch.object(geometry, 'compose', wraps=geometry.compose) as spy:
            np.testing.assert_array_equal(self.pixels(self.effect.poster_frame), complete)
